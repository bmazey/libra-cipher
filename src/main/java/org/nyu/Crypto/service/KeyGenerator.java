...
public class KeyGenerator {
	...
	public HashMap<String, ArrayList<Integer>> generatePutativeKey(int[] ciphertext) {
		...
		// The first 3 characters cannot be space as the shortest word possible is of
		// length 3
		...
		// We add all the other characters of the ciphertext to a "whitelist", all the
		...
		// If 2 consecutive characters are equal, we assume there is a high possibility
		// that it is a "b"
		...
		// If a character is a space, the 3 following characters as well as the 3
		// preceding characters cannot be space
		...
		while (spaceTemp != spaceFrequency) {
			...
			for (int i = 3; i < ciphertext.length - 3; i++) {
				if (spaceValues.contains(ciphertext[i])) {
					if (spaceValues.contains(ciphertext[i + 1]) || spaceValues.contains(ciphertext[i + 2])
							|| spaceValues.contains(ciphertext[i + 3])) {
						break;
					} else if (spaceValues.contains(ciphertext[i - 1]) || spaceValues.contains(ciphertext[i - 2])
							|| spaceValues.contains(ciphertext[i - 3])) {
						break;
					} else
						spaceTemp++;
		...
		// The 86 numbers left are shuffled randomly one more time before being assigned
		// to the putative key
